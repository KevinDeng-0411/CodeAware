...
结构化输出（with_structured_output + json_mode），失败回退 ainvoke + 容错 JSON 解析。
"""
...
async def evaluate_draft(model, question: str, draft: str) -> ReflectionVerdict:
    ...
    # 优先：with_structured_output（json_mode）
    try:
        structured = model.with_structured_output(ReflectionVerdict, method="json_mode")
