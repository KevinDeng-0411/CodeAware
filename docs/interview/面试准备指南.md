...
| `context.references` | 知识库命中的引用列表（前端可展示引用来源，配合 v2 引用纪律） |
| `reasoning.delta` | 模型思考过程增量（reasoning_content；Agent Reflection 的 draft 也走它） |
| `token.delta` | 模型原始非空 chunk，不 trim |
| `tool.call` | Agent 选中并调用工具（工具名 + 参数，回放/前端展示用） |
| `tool.result` | 工具执行结果（`status: ok/error`） |
...
> 完整共 10 个事件类型（对齐 `app/schemas/chat_events.py`）。其中 reasoning/tool 事件只在
> Agent 模式与 Reflection/多阶段推理时出现，RAG 纯文本问答的常规流是前 4 + token + completed/failed。
...
**长程任务的思考：为什么是 checkpoint 而不是摘要压缩（评估后暂缓）**

长程任务（一个目标跨很多轮/很多小时持续推进）要的不是"记住发生了什么"
（摘要），而是"现在在哪、做过什么、下一步干什么"（**任务状态**）。倾向的方案：

- **窗口硬切分换新窗口**，不做压缩——窗口满了切新窗口重新进入上下文；
- **note 结构化记录用户状态**（类似 checkpoint，`with_structured_output` 在明确
  事件点生成），按 **已完成 / 进行中 / 下一步** 三部分结构化写；
- **history 全量存档外部**——新窗口一问时按需检索回填，而不是整段塞回上下文。

**三个必防的坑**：① checkpoint 要用显式 schema 在明确事件点生成，不能靠模型自由发挥；
② history 是"检索回填"不是"整段塞回"（否则等于没切窗口）；③ note 要带来源引用可回查，
否则"未完成项被写错"不可逆。

**当前未落地原因**：RAG 模式是问答（无"任务"概念）、Agent 模式是单轮有界
（`MAX_STEPS=4`，turn 结束即落库）——两个模式都没有"长程任务"这个触发条件。
按"评估后暂缓、条件满足再落地"惯例（同 reranker / checkpoint / 实时高亮）：
**只在 Agent 方向预留**。触发条件：① 出现用户跨多会话持续推进同一真实目标；
② Agent 升级为持久目标代理。条件未满足前，它是未来预留，不是当前债务。
（与 [深挖</think>④ 追问 5](面试深挖-简历逐条.md) 双向可追溯）
...
### 异步任务的可靠性（Celery / Kafka 分工）

**broker 选型**：Celery 用 **Redis 当 broker**（复用现有缓存组件，不引入 RabbitMQ），默认 **Redis list**（LPUSH/BRPOP）——这是 Celery 的成熟实现，ack、重投、visibility 已建在 list 之上；stream 的消费者组/逐条 ack 对 Celery 是重复的，单机量级 list 够用。

**为什么不用 Kafka 当任务队列**：Kafka 的 **at-least-once ≠ 任务执行保证**——它只保"不丢传递"，代价是"可能重放"。恰逢文档分块/记忆抽取**是重放会生产重复数据**的操作，所以"不重复"仍靠应用层幂等（先查是否已分块）。既然不丢靠 ack、不重复靠幂等，两套在此点等价；Kafka 强的是**事件流吞吐/重放/广播**，对单机几百条/天的任务量大炮打蚊子，还要另起组件。

**任务 vs 事件分工**：Celery = **任务**（要做什么，需重试/幂等/结果）→ 文档分块、记忆抽取；Kafka = **事件**（发生了什么，可观测/审计/metrics counter）→ audit/metrics。按两个角色分开选型。

**幂等的真实深度（诚实）**：文档分块是**真幂等**（业务键存在性检查吸收重放）；记忆抽取是**半幂等**（`has_memories` 早退挡重入，但 `save_prepared_facts` 仍盲插，无 dedup/conflict）——半幂等的缺口已列入 B 层改进（冲突感知 save）。(与 [深挖 1.1-1.3](面试深挖-简历逐条.md) 呼应)
