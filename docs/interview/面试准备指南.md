...
| 项目 | 结果 |
|---|---|
| 后端全量 | `350 passed, 0 failed`（Celery 异步 + Kafka + LangGraph + reranker + Agent LLMOps） |
| 后端覆盖率 | 高（run_tests_safe --cov 可复测） |
| 前端测试 | `43 passed` |
| 前端 lint/build | 通过 |
| Alembic | 唯一 `head=current=0012` |
...
| 单腿候选 | `top_k * 3` |
| RRF | `1/(k+rank)`，k=60 |
| 后端测试 | 350 passed, 0 failed（含 reranker + LangGraph + Kafka + Celery + Agent LLMOps） |
...
| 长期领域决策 | `docs/decisions/adr/0001~0017` |
