...
- **编排迁 LangGraph**（[ADR-0018](../decisions/adr/0018-agent-react-langgraph.md)）：主循环从手写 while 迁到 StateGraph（`agent_graph.py`，`react_loop.py` 变薄壳、签名/SSE 契约不变）——agent/tools 节点 + 条件边表达工具循环，收敛检测/防打转/per-tool 上限语义逐一保真；轻量 **Reflection** 节点（生成后自评，不达标注入 feedback 再生成）默认关。动因：面试叙事（主流编排标准）+ 可维护性（读图 vs 读手写循环）+ 扩展性（ADR-0016 预留的"多工具复杂度上升 LangGraph 更优"条件满足）。
...
| 项目 | 结果 |
|---|---|
| 后端全量 | `353 passed, 0 failed`（Celery 异步 + Kafka + LangGraph + reranker + Agent LLMOps） |
...
| 单腿候选 | `top_k * 3` |
| RRF | `1/(k+rank)`，k=60 |
| 后端测试 | 353 passed, 0 failed（含 reranker + LangGraph + Kafka + Celery + Agent LLMOps） |
